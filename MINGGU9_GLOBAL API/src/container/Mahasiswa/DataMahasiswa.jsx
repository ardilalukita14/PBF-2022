...
class DataMahasiswa extends Component {
    state = {                        // komponen state dari React untuk statefull component
        ...
        insertMahasiswa: {               // variabel yang digunakan untuk menampung sementara data yang akan di insert
            nim: 2041720056,             // kolom userId, id, title, dan body sama, mengikuti kolom yang ada pada listMahasiswa.json
            id: 1,
            nama: "",
            alamat: "",
            hp: "",
    ...
    ambilDataDariServerAPI = () => {
        fetch('http://localhost:3002/mahasiswa')  // alamat URL API yang ingin kita ambil datanya
            ...
            .then(jsonHasilAmbilDariAPI => {    // data json hasil ambil dari API kita masukkan ke dalam listMahasiswa pada state 
                this.setState({
    ...
    handleTambahMahasiswa = (event) => {      // fungsi untuk meng-handle form tambah data Mahasiswa
        ...
        formInsertMahasiswa[event.target.name] = event.target.value;      // menyimpan data onchange ke formInsertMahasiswa sesuai dengan target yang diisi
        this.setState({
